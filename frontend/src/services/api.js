...
export const API = {
    ...
    // AI Hospital Chatbot (Django / Laravel)
    queryChatbot: async (message) => {
        try {
            // Attempt calling Python Django Chatbot Service on port 8001
            return await axios.post('http://localhost:8001/api/chatbot/query/', { message }, { timeout: 3000 });
        } catch (e) {
            // Fallback to Laravel backend API if Django server is not active
            return await axiosInstance.post('/chatbot/query', { message });
        }
    },
};
